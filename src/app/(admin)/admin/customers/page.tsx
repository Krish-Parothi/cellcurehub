...
const statusColor = (s: string) => {
  const m: Record<string, string> = {
    ...
    repair_in_progress: 'bg-orange-500/20 text-orange-400', done: 'bg-green-500/20 text-green-400',
  ...
  return (
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-white/50 text-sm mt-1">Customer database & profiles</p>
      </motion.div>

      <div className="relative max-w-sm">
        <Search className="absolute left-3 top-2.5 w-4 h-4 text-white/40" />
        <Input className="pl-9 bg-white/5 border-white/10 text-white" placeholder="Search by name, phone, or email..." value={search} onChange={e => setSearch(e.target.value)} />
      </div>
      ...
      <Card className="bg-white/5 border-white/10"><CardContent className="p-0">
        {loading ? <div className="p-6"><Skeleton className="h-64 w-full bg-white/5" /></div> : (
          <Table><TableHeader><TableRow className="border-white/5 hover:bg-transparent">
            ...
            <TableHead className="text-white/50">Email</TableHead><TableHead className="text-white/50">Repairs</TableHead>
            <TableHead className="text-white/50">Spent</TableHead><TableHead className="text-white/50">Last Repair</TableHead>
          ...
          <TableBody>{filtered.map(c => (
            <TableRow key={c.id} className="border-white/5 hover:bg-white/5">
              <TableCell className="flex items-center gap-2">
                <Avatar className="h-7 w-7"><AvatarFallback className="bg-[#00D084]/20 text-[#00D084] text-xs">{c.full_name?.[0] || '?'}</AvatarFallback></Avatar>
                <span className="text-white">{c.full_name}</span>
              </TableCell>
              <TableCell className="text-white/60">{c.phone || '—'}</TableCell>
              <TableCell className="text-white/60 text-xs">{c.email || '—'}</TableCell>
          ...
          {selectedCustomer && (
            <>
              <SheetHeader className="mb-4">
                <SheetTitle className="text-white flex items-center gap-2"><UserCircle className="w-5 h-5 text-[#00D084]" />{selectedCustomer.full_name}</SheetTitle>
                <SheetDescription className="text-white/50">{selectedCustomer.email}</SheetDescription>
              </SheetHeader>
              <div className="space-y-6 pb-12">
                <div className="grid grid-cols-2 gap-3 text-sm">
                  ...
                  <div className="bg-white/5 p-3 rounded-lg"><span className="text-white/40 text-xs block">Total Repairs</span><p className="text-white text-lg font-bold">{selectedCustomer.repairCount}</p></div>
                  <div className="bg-white/5 p-3 rounded-lg"><span className="text-white/40 text-xs block">Total Spent</span><p className="text-[#00D084] text-lg font-bold">₹{fmt(selectedCustomer.totalSpent)}</p></div>
                </div>

                <Separator className="bg-white/10" />

                <div>
                  <p className="text-xs text-white/60 mb-2 font-semibold">Repair History</p>
                  ...
                  ))}{customerRepairs.length === 0 && <p className="text-white/20 text-center py-4">No repairs</p>}</div>
                </div>

                {customerReviews.length > 0 && (
                  <div>
                    <p className="text-xs text-white/60 mb-2 font-semibold">Reviews</p>
                    <div className="space-y-2">{customerReviews.map(rv => (
                      <div key={rv.id} className="bg-white/5 rounded-lg p-3 text-sm">
                        <div className="flex items-center gap-1 text-amber-400 text-xs mb-1">{'★'.repeat(rv.rating)}{'☆'.repeat(5 - rv.rating)}</div>
                        <p className="text-white/70">{rv.comment}</p>
                      </div>
                    ))}</div>
                  </div>
                )}

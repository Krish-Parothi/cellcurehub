...
import { Recycle, Eye, Loader2, CheckCircle2, IndianRupee, MapPin } from 'lucide-react';
...
export default function AdminEwastePage() {
  ...
  useEffect(() => {
    if (user?.role === 'admin') {
      fetchData();
      supabase.from('users').select('id, full_name, role').in('role', ['admin', 'delivery']).then(({ data }) => setDeliveryBoys(data || []));
    }
  }, [user, fetchData]);

  const acceptPrice = async (item: any) => {
    setUpdating(item.id);
    try {
      const { error } = await supabase.from('ewaste').update({ status: 'admin_offered', admin_offer: item.requested_price }).eq('id', item.id);
      if (error) throw error;
      toast.success('Accepted customer price. Waiting for their confirmation.');
      fetchData();
  ...
  const sendCounterOffer = async (item: any) => {
    if (!counterOffer || isNaN(Number(counterOffer))) { toast.error('Enter valid counter offer'); return; }
    setUpdating(item.id);
    try {
      const { error } = await supabase.from('ewaste').update({ status: 'admin_offered', admin_offer: Number(counterOffer) }).eq('id', item.id);
      if (error) throw error;
  ...
  const assignDelivery = async (item: any) => {
    ...
    try {
      ...
      toast.error(err.message || 'Failed to assign delivery. Make sure DB schema is updated.');
    } finally {
      setUpdating(null);
    }
  };
    ...
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} className="flex flex-col sm:flex-row sm:justify-between sm:items-end gap-4">
        ...
        <div className="flex flex-wrap gap-2">
          {['all', 'pending', 'admin_offered', 'agreed', 'picked_up', 'credited', 'rejected'].map(filter => (
            <Button 
              key={filter} 
                    ...
                    <TableRow key={sub.id} className="border-[#E8E4DF] hover:bg-[#F7F7F5]">
                      ...
                      <TableCell>
                        {sub.quoted_value ? (
                          ...
                          <span className="text-[#1A1A1A]/70 font-medium flex items-center text-sm">
                            <IndianRupee className="w-3 h-3 mr-0.5" />{fmt(sub.requested_price)} (Req)
                          </span>
                        ) : (
                          <span className="text-[#1A1A1A]/40 text-sm">Not locked</span>
                        )}
                      </TableCell>
                      <TableCell>
        ...
        <DialogContent className="bg-[#F7F7F5] border-[#E8E4DF] max-w-2xl shadow-lg max-h-[90vh] overflow-y-auto">
          <DialogHeader>
            ...
            <DialogDescription className="text-[#1A1A1A]/60">
              Review device condition and negotiate pricing.
            </DialogDescription>
          </DialogHeader>
            ...
            <div className="space-y-4 mt-2">
              ...
              {detailsDialog.item.status === 'pending' && (
                <div className="bg-indigo-50 p-4 rounded-xl border border-indigo-100 space-y-4">
                  <h4 className="font-semibold text-indigo-900">Price Negotiation</h4>
                  <p className="text-sm text-indigo-800">Customer requested: <span className="font-bold text-indigo-600 text-lg">₹{detailsDialog.item.requested_price}</span></p>
                  <div className="flex gap-2">
                    <Button onClick={() => acceptPrice(detailsDialog.item)} disabled={updating === detailsDialog.item.id} className="bg-emerald-600 hover:bg-emerald-700 text-white">Accept Customer Price</Button>
                  </div>
                  <div className="pt-2 border-t border-indigo-200">
                    <Label className="text-indigo-900 mb-2 block">Or Send Counter Offer (₹)</Label>
                    <div className="flex gap-2 items-center">
                      <Input type="number" value={counterOffer} onChange={e => setCounterOffer(e.target.value)} placeholder="Enter amount" className="bg-white text-[#1A1A1A]" />
                      <Button onClick={() => sendCounterOffer(detailsDialog.item)} disabled={updating === detailsDialog.item.id} variant="outline" className="border-indigo-300 text-indigo-700 bg-white">Send Offer</Button>
                    </div>
                  </div>
                </div>
              )}
              ...
              {detailsDialog.item.status === 'agreed' && (
                <div className="bg-emerald-50 p-4 rounded-xl border border-emerald-100 space-y-4">
                  <h4 className="font-semibold text-emerald-900 flex items-center"><CheckCircle2 className="w-4 h-4 mr-1" /> Price Agreed</h4>
                  <p className="text-sm text-emerald-800">Final Price: <span className="font-bold">₹{detailsDialog.item.quoted_value}</span></p>
                  
                  <div className="pt-2 border-t border-emerald-200">
                    <Label className="text-emerald-900 mb-2 block">Assign Delivery Boy for Pickup</Label>
                    <div className="flex gap-2 items-center">
                ...
                <div className="flex gap-2 items-center">
                  <Select value={detailsDialog.item.status} onValueChange={(v) => forceUpdateStatus(detailsDialog.item.id, v)}>
                    ...
                    <SelectContent className="bg-white text-[#1A1A1A] border-[#E8E4DF]">
                      {['pending', 'admin_offered', 'agreed', 'rejected', 'valued', 'picked_up', 'credited'].map(status => (

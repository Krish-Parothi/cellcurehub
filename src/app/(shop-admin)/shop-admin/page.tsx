...
export default function ShopAdminDashboard() {
  ...
  const statCards = [
    ...
    { icon: FileSearch, label: 'Pending RCA', value: stats.pendingRca, color: 'text-purple-400' },
  ];
  ...
  return (
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-white/50 text-sm mt-1">Your shop&apos;s real-time overview</p>
      </motion.div>
          ...
          <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.05 }}>
            <Card className="bg-white/5 border-white/10 hover:border-[#00D084]/20 transition-colors">
              <CardContent className="p-5 flex items-center justify-between">
                <div><p className="text-xs text-white/50">{s.label}</p><p className="text-2xl font-bold text-white mt-1">{loading ? '...' : s.value}</p></div>
      ...
      <Card className="bg-white/5 border-white/10">
        <CardHeader className="pb-3"><CardTitle className="text-white text-sm flex items-center gap-2"><Activity className="w-4 h-4 text-[#00D084]" /> Live Activity Feed</CardTitle></CardHeader>
            ...
            <div className="max-h-80 overflow-y-auto">
              {timeline.map((t: any) => (
                <div key={t.id} className="flex items-center gap-3 px-4 py-3 border-b border-white/5 hover:bg-white/[0.02] text-sm">
                  ...
                  <span className="text-white/80 truncate flex-1">{t.repair?.customer?.full_name || 'Customer'} — {t.repair?.device?.brand} {t.repair?.device?.model_name}</span>
      ...
      {/* Follow-up Queue */}
      <Card className="bg-white/5 border-white/10">
        <CardHeader className="pb-3"><CardTitle className="text-white text-sm flex items-center gap-2"><MessageSquare className="w-4 h-4 text-amber-400" /> 48-Hour Follow-ups ({followUps.length})</CardTitle></CardHeader>
        <CardContent className="p-0">
          {loading ? <div className="p-4"><Skeleton className="h-20 w-full bg-white/5" /></div> : followUps.length === 0 ? (
            ...
            <Table><TableHeader><TableRow className="border-white/5 hover:bg-transparent">
              ...
              <TableHead className="text-white/50">Device</TableHead><TableHead className="text-white/50">Delivered</TableHead>
            ...
            <TableBody>{followUps.map((r: any) => (
              <TableRow key={r.id} className="border-white/5 hover:bg-white/5">
                ...
                <TableCell><Button size="sm" onClick={() => sendFollowUp(r)} className="bg-green-600 hover:bg-green-700 text-white text-xs"><MessageSquare className="w-3 h-3 mr-1" />WhatsApp</Button></TableCell>
              </TableRow>
            ))}</TableBody></Table>
          )}
        </CardContent>

  ...
  return (
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-white/50 text-sm mt-1">Your shop&apos;s parts and items</p>
      </motion.div>

      <Tabs defaultValue="parts" className="w-full">
        <TabsList className="bg-white/5 border border-white/10 mb-6">
          <TabsTrigger value="parts" className="data-[state=active]:bg-[#00D084]/15 data-[state=active]:text-[#00D084]"><Package className="w-3.5 h-3.5 mr-1.5" />Parts</TabsTrigger>
          <TabsTrigger value="items" className="data-[state=active]:bg-[#00D084]/15 data-[state=active]:text-[#00D084]"><ShoppingBag className="w-3.5 h-3.5 mr-1.5" />Shop Items</TabsTrigger>
        </TabsList>

        <TabsContent value="parts">
          <div className="flex justify-end mb-4"><Button onClick={() => { setPartForm({ name: '', brand: '', model_compatible: '', quantity_in_stock: 0, cost_price: 0, selling_price: 0, low_stock_threshold: 5 }); setPartDialog({ open: true, part: null }); }} className="bg-[#00D084] text-black hover:bg-[#00D084]/90"><Plus className="w-4 h-4 mr-1" />Add Part</Button></div>
          <Card className="bg-white/5 border-white/10"><CardContent className="p-0">
            {loading ? <div className="p-6"><Skeleton className="h-48 w-full bg-white/5" /></div> : (
              ...
              <TableBody>{parts.map(p => {
                ...
                return (<TableRow key={p.id} className={`border-white/5 ${low ? 'bg-red-500/5' : 'hover:bg-white/5'}`}>
        ...
        <TabsContent value="items">
          <div className="flex justify-end mb-4"><Button onClick={() => { setItemForm({ name: '', category: '', price: 0, stock_qty: 0, image: null }); setItemDialog({ open: true, item: null }); }} className="bg-[#00D084] text-black hover:bg-[#00D084]/90"><Plus className="w-4 h-4 mr-1" />Add Item</Button></div>
          <Card className="bg-white/5 border-white/10"><CardContent className="p-0">
            {loading ? <div className="p-6"><Skeleton className="h-48 w-full bg-white/5" /></div> : (
              <Table><TableHeader><TableRow className="border-white/5 hover:bg-transparent">
              ...
              <TableBody>{shopItems.map(si => (
                <TableRow key={si.id} className="border-white/5 hover:bg-white/5">
                  ...
                  <TableCell className="flex gap-1">
                    <Button size="icon" variant="ghost" className="h-7 w-7 text-white/60 hover:text-[#00D084]" onClick={() => { setItemForm({ name: si.name, category: si.category || '', price: si.price, stock_qty: si.stock_qty, image: null }); setItemDialog({ open: true, item: si }); }}><Pencil className="w-3 h-3" /></Button>
      ...
      <Dialog open={partDialog.open} onOpenChange={o => setPartDialog({ open: o, part: null })}>
        <DialogContent className="bg-[#1A1A1A] border-white/10 max-w-md">
          <DialogHeader><DialogTitle className="text-white">{partDialog.part ? 'Edit' : 'Add'} Part</DialogTitle><DialogDescription className="text-white/50">For your shop only</DialogDescription></DialogHeader>
      ...
      <Dialog open={itemDialog.open} onOpenChange={o => setItemDialog({ open: o, item: null })}>
        <DialogContent className="bg-[#1A1A1A] border-white/10 max-w-md">
          <DialogHeader><DialogTitle className="text-white">{itemDialog.item ? 'Edit' : 'Add'} Shop Item</DialogTitle><DialogDescription className="text-white/50">For your shop only</DialogDescription></DialogHeader>
          <div className="space-y-3">

  ...
  return (
    <div className="space-y-8">
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }}>
        ...
        <p className="text-[#1A1A1A]/60 text-sm mt-1">View supported devices across the platform</p>
      </motion.div>

      <div className="bg-blue-500/10 border border-blue-500/20 rounded-xl p-4 flex items-start gap-3">
        <Info className="w-5 h-5 text-blue-600 shrink-0 mt-0.5" />
        <div>
          <p className="text-sm font-semibold text-blue-600">Read-Only View</p>
          <p className="text-xs text-blue-600/80 mt-1">Device models and categories are managed globally by the main administrator. If you need a new model added, please contact the admin.</p>
        </div>
      </div>
            ...
            <Collapsible key={brand} open={expandedBrands.has(brand)} onOpenChange={() => toggleBrand(brand)}>
              <Card className="bg-white border-[#E8E4DF] shadow-sm">
                <CollapsibleTrigger className="w-full">
                  <CardContent className="p-4 flex items-center justify-between cursor-pointer hover:bg-[#F7F7F5]">
                    <div className="flex items-center gap-3">
                      <Smartphone className="w-4 h-4 text-[#FF5C00]" />
                      <span className="text-[#1A1A1A] font-semibold">{brand}</span>
                      <Badge variant="outline" className="border-[#E8E4DF] text-[#1A1A1A]/50 text-xs">{models.length}</Badge>
                    </div>
                ...
                <CollapsibleContent>
                  <div className="border-t border-[#E8E4DF]/60">
                    {models.map(d => (
                      <div key={d.id} className="flex items-center justify-between px-4 py-3 border-b border-[#E8E4DF]/60 last:border-0">
                        <div className="flex items-center gap-3">
                          <span className={`text-sm ${d.is_active ? 'text-[#1A1A1A]' : 'text-[#1A1A1A]/30 line-through'}`}>{d.model_name}</span>
                          <Badge className="bg-[#F7F7F5] border border-[#E8E4DF] text-[#1A1A1A]/50 text-[10px] capitalize">{d.category}</Badge>
                        </div>
                        {!d.is_active && <Badge variant="outline" className="text-red-600 border-red-500/20 bg-red-500/10 text-[10px]">Inactive</Badge>}
                      </div>
                    ))}
                  </div>
                </CollapsibleContent>
